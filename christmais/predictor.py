...
class Predictor: 
	...
	def __init__(self, seed):
		...
		# def _get_labels()
		self.labels_url = 'https://s3.amazonaws.com/outcome-blog/imagenet/labels.json'
		self.labels = {int(key): value.split(", ") for (key, value) in requests.get(self.labels_url).json().items()}
	...
	def get_score(self, inp_file, target_label):
	    ...
	    indices = [index for index, labels in self.labels.items() if target_label == labels[0]]
	    ...
	    for model_name, model in models.items():
	        ...
	        result = {label[0]: probs[0][index] for index, label in self.labels.items()}
